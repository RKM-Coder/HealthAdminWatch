...
public class TalukListActivity extends BaseActivity implements TalukListAdapter.OnClickMainView {
    ...
    private TalukListAdapter adapter;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        tvHeaderFac.setText("List of Taluk");
    ...
    List<BlockModel> listOfTaluk;

    private void getTaluk() {
        String jsonFileString = getCommonApi().getJsonFromAssets(getApplicationContext(), "blockjson_new.json");
    ...
    private void defaluttaluk() {
        ...
        List<BlockModel> listOfTalukBydistrict = new ArrayList<>();
        for (BlockModel talukModel : listOfTaluk) {
            if (talukModel.getDistrictCode().equalsIgnoreCase(dftdistrict_code)) {
                listOfTalukBydistrict.add(talukModel);
            }
        }
        adapter.addall((ArrayList<BlockModel>) listOfTalukBydistrict);
        // makeSpinnerTalukMethod((ArrayList<TalukModel>) listOfTalukBydistrict);
    }

    private void initrecyclerView() {
        adapter = new TalukListAdapter(this, new ArrayList<BlockModel>());
    ...
    private void initMvp() {
        ...
        networkService.inject(TalukListActivity.this);
    }
    ...
    @Override
    public void onClickMain(int position, BlockModel item) {
    ...
    public void onBackPressed() {
        ...
        }else if (key.equalsIgnoreCase("taluk")) {
            String message = "";
            intent.putExtra("name", message);
            intent.putExtra("name_id", "");
            setResult(RES_TALUK, intent);
        }else if (key.equalsIgnoreCase("panchyate")) {
            String message = "";
            intent.putExtra("name", message);
            intent.putExtra("name_id", "");
            setResult(RES_PANCHYATE, intent);
